...
Java_com_example_newplayer_FFPlayerKT_prepareNative(JNIEnv *env, jobject thiz, jstring data_source) {
    ...
    auto *player = new FFPlayer(data_source_, helper); // 有意为之的，开辟堆空间，不能释放
...
Java_com_example_newplayer_FFPlayerKT_stopNative(JNIEnv *env, jobject thiz, jlong native_obj) {
    auto *player = reinterpret_cast<FFPlayer *>(native_obj);
    if (player) {
        player->stop();
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_newplayer_FFPlayerKT_startNative(JNIEnv *env, jobject thiz, jlong native_obj) {
    auto *player = reinterpret_cast<FFPlayer *>(native_obj);
    if (player) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_newplayer_FFPlayerKT_setSurfaceNative(JNIEnv *env, jobject thiz, jobject surface, jlong native_obj) {
    auto *player = reinterpret_cast<FFPlayer *>(native_obj);
    if (!player) {
        LOGE("player is null");
        return;
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_newplayer_FFPlayerKT_releaseNative(JNIEnv *env, jobject thiz, jlong native_obj) {
    auto *player = reinterpret_cast<FFPlayer *>(native_obj);
    if (!player) {
        LOGE("player is null");
        return;
